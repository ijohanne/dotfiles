...
in
{
  config = mkIf (pkgs.stdenv.isLinux && pkgs.stdenv.isx86_64 && cfg.enable) (mkMerge [
    {
      home.packages = with pkgs; [ haskell.compiler.ghc922 ] ++ (with pkgs.haskellPackages; [ cabal-install stack ]);
    }
    (mkIf config.dotfiles.development-tools.neovim.language-servers.enable
      {
        ...
        dotfiles.development-tools.neovim.language-servers = {
          extraLua = ''
            lspconfig['hls'].setup {
              on_attach = on_attach,
              cmd = {"${pkgs.haskell-language-server}/bin/haskell-language-server-9.2.2", "--lsp" }
            }
          '';
        };
